...
function usage() {
  console.error("Usage: node logic/tools/ontology_graph_to_pgm.mjs <ontology_graph.ndjson> [out.pgm]");
}

function main() {
  const inPath = process.argv[2];
  const outPath = process.argv[3] || path.resolve("logic/generated/ontology_graph.pgm");
  if (!inPath) {
    usage();
    process.exit(2);
  }
