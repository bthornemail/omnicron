...
/* ------------------------------------------------------------
 * UTF-8 normalization for Aegean headers and 6-dot Braille pairs
 *
 * Lexer remains ASCII-token based. We normalize:
 * - U+10100..U+1013F -> aegean_header_xx
 * - U+2800..U+283F   -> braille6_xx
 * - consecutive braille6 cells -> braille6_pair_xx_yy
 * ------------------------------------------------------------ */
static bool utf8_decode_one(const char* s, int pos, uint32_t* cp, int* width) {
    ...
    if ((p[0] & 0xF8u) == 0xF0u && p[1] != 0 && p[2] != 0 && p[3] != 0) {
...
static bool is_aegean_header_cp(uint32_t cp) {
    return cp >= 0x10100u && cp <= 0x1013Fu;
}

static bool is_braille6_cp(uint32_t cp) {
    return cp >= 0x2800u && cp <= 0x283Fu;
}

static char* normalize_header_stream(const char* input) {
    size_t in_len = strlen(input);
    size_t cap = in_len * 8 + 64; /* expansion buffer */
    char* out = malloc(cap);
    if (!out) {
        fprintf(stderr, "out of memory\n");
        exit(1);
    ...
    for (int i = 0; input[i] != 0; ) {
        uint32_t cp = 0;
        int w = 0;
        if (!utf8_decode_one(input, i, &cp, &w)) break;

        if (is_aegean_header_cp(cp)) {
            char buf[64];
            uint32_t v = cp - 0x10100u;
            int n = snprintf(buf, sizeof(buf), " aegean_header_%02x ", (unsigned)v);
            if (oi + (size_t)n + 1 >= cap) {
                cap *= 2;
                out = realloc(out, cap);
                if (!out) exit(1);
            }
        ...
        if (is_braille6_cp(cp)) {
            uint32_t cp2 = 0;
            int w2 = 0;
            if (utf8_decode_one(input, i + w, &cp2, &w2) && is_braille6_cp(cp2)) {
                char buf[96];
                uint32_t a = cp - 0x2800u;
                uint32_t b = cp2 - 0x2800u;
                int n = snprintf(buf, sizeof(buf), " braille6_pair_%02x_%02x ", (unsigned)a, (unsigned)b);
                if (oi + (size_t)n + 1 >= cap) {
                    cap *= 2;
                    out = realloc(out, cap);
                    if (!out) exit(1);
                ...
                char buf[64];
                uint32_t a = cp - 0x2800u;
                int n = snprintf(buf, sizeof(buf), " braille6_%02x ", (unsigned)a);
                if (oi + (size_t)n + 1 >= cap) {
                    cap *= 2;
                    out = realloc(out, cap);
                    if (!out) exit(1);
        ...
        if (oi + (size_t)w + 1 >= cap) {
            cap *= 2;
            out = realloc(out, cap);
            if (!out) exit(1);
        }
        memcpy(out + oi, input + i, (size_t)w);
        oi += (size_t)w;
        i += w;
    }
...
void tokenize(Tokenizer* tz) {
    ...
    char* normalized = normalize_header_stream(tz->input);
